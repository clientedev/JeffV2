...
def importar_linha_tecnologia(db: Session):
    ...
    try:
        ...
        df = pd.read_excel('attached_assets/linha tecnologia_1761070543800.xlsx')
        ...
        for _, row in df.iterrows():
            try:
                registro = LinhaTecnologia(
                    ...
                    dados_socios_informados=safe_str(row.get('S/N          '), 50),
                    contrato_enviado_empresa=safe_str(row.get('S/N            '), 50),
                    contrato_assinado_empresa=safe_str(row.get('S/N           '), 50),
                    data_contrato_assinado_senai=safe_date(row.get('DATA DO CONTRATO ASSINADO SENAI')),
                    data_cadastro_sgt=safe_date(row.get('DATA CADASTRO SGT')),
                    data_upload_sgt=safe_date(row.get('DATA UPLOAD SGT')),
                    data_aceite_sgt=safe_date(row.get('DATA ACEITE SGT')),
                    data_envio_relatorio_t1=safe_date(row.get('DATA DO ENVIO DO RELATÓRIO T1')),
                    data_cobranca_empresa=safe_date(row.get('DATA DE COBRANÇA DA EMPRESA')),
                    cadastro_plataforma_ok=safe_str(row.get('S/N       '), 50),
                    data_cobranca_sebrae=safe_date(row.get('DATA COBRANÇA SEBRAE ')),
                    contrato_sebrae_enviado=safe_str(row.get('S/N         '), 50),
                    data_resposta_sebrae=safe_date(row.get('DATA RESPOSTA SEBRAE')),
                    requisicao_grm=safe_str(row.get('REQUISIÇÃO GRM'), 100),
                    data_inicio=safe_date(row.get('DATA INÍCIO')),
                    data_termino=safe_date(row.get('DATA TÉRMINO')),
                    reuniao_kickoff_confirmada=safe_str(row.get('REUNIÃO KICK-OFF CONFIRMADA'), 50),
                    reuniao_final_agendada=safe_str(row.get('REUNIÃO FINAL AGENDADA'), 50),
                    ...
                    mov_1_1_57=safe_str(row.get('MOV. 1.1.57'), 50),
                    relatorio_priorizacao_enviado=safe_str(row.get('RELATÓRIO DE PRIORIZAÇÃO ENVIADO'), 50),
                    relatorio_smart_factory=safe_str(row.get('RELATÓRIO SMART FACTORY'), 50),
                    relatorio_final_enviado=safe_str(row.get('RELATÓRIO FINAL ENVIADO'), 50),
                    relatorio_educacional_enviado=safe_str(row.get('RELATÓRIO EDUCACIONAL ENVIADO'), 50),
                    data_conclusao_sgt=safe_date(row.get('DATA DE CONCLUSÃO SGT')),
                    envio_auditoria=safe_str(row.get('S/N                            '), 50),
                    retorno_auditoria=safe_str(row.get('S/N                             '), 50),
                    data_prestacao_contas=safe_date(row.get('DATA PRESTAÇÃO DE CONTAS')),
                    data_pesquisa_satisfacao=safe_date(row.get('DATA PESQUISA DE SATISFAÇÃO RESPONDIDA')),
                    observacoes=safe_str(row.get('OBSERVAÇÕES')),
                    especifico_saldo=safe_str(row.get('ESPECÍFICO (SALDO SGSET AGOSTO)'), 255),
                    passou_ali_2023=safe_str(row.get('Passou no ALI em 2023 ? '), 50),
                    ali_2024_convidar=safe_str(row.get('ALI 2024 - Convidar ?'), 50),
                    t3_solucao_1_2024=safe_str(row.get('T 3 (1ª  Solução Proposta para 2024 )')),
                    t3_solucao_2_2024=safe_str(row.get('T 3 (2ª  Solução Proposta para 2024 )')),
                    aceitou=safe_str(row.get('ACEITOU ?'), 50),
                    qual_3_etapa=safe_str(row.get('QUAL ? (           3ª ETAPA)')),
                    t4_proposto_2024=safe_str(row.get('T4 Proposto para 2024 ')),
                    aceitou_2=safe_str(row.get('ACEITOU ?2'), 50),
                    follow_2024=safe_str(row.get('FOLLOW 2024'), 255),
                    continuidade=safe_str(row.get('CONTINUIDADE'), 255),
                    etapa_5_ou_6=safe_str(row.get('5ª ou 6ª etapa'), 255),
                    fez_efici_energ=safe_str(row.get('FEZ EFICI ENERG ?'), 50),
                    obs2=safe_str(row.get('OBS2')),
                    ano=safe_int(row.get('ANO')),
                    mes=safe_str(row.get('MÊS'), 20),
                    inu=safe_str(row.get('INU'), 50),
                    hubspot=safe_str(row.get('HUBSPOT'), 50),
                    nova_proposta=safe_str(row.get('NOVA PROPOSTA'), 50),
                    valor_sap=safe_numeric(row.get('VALOR SAP')),
                    dados_iniciais=True
                )
                db.add(registro)
        ...
        for _, row in df.iterrows():
            try:
                ...
                registro = LinhaEducacional(
                    linha=safe_str(row.get('LINHA'), 100),
                    tipo=safe_str(row.get('TIPO'), 100),
                    cnpj=safe_str(row.get('CNPJ'), 18),
                    cliente=safe_str(row.get('CLIENTE'), 255),
                    estabelecimento=safe_str(row.get('ESTABELECIMENTO'), 255),
                    numero_cotacao=safe_str(row.get('Nº COTAÇÃO'), 50),
                    data_envio_cotacao=safe_date(row.get('DATA ENVIO COTAÇÃO')),
                    data_follow=safe_date(row.get('DATA FOLLOW')),
                    status_cotacao=safe_str(row.get('STATUS COTAÇÃO'), 100),
                    motivo=safe_str(row.get('MOTIVO'), 255),
                    programa=safe_str(row.get('PROGRAMA'), 255),
                    cfp_proposta=safe_str(row.get('CFP PROPOSTA'), 100),
                    cfp_parceiro=safe_str(row.get('CFP PARCEIRO'), 100),
                    modalidade=safe_str(row.get('MODALIDADE'), 100),
                    ch=safe_str(row.get('CH'), 50),
                    turno=safe_str(row.get('TURNO'), 50),
                    data_inicio=safe_date(row.get('INÍCIO')),
                    data_termino=safe_date(row.get('TÉRMINO')),
                    numero_proposta=safe_str(row.get('Nº PROPOSTA'), 50),
                    numero_ordem_venda=safe_str(row.get('Nº ORDEM DE VENDA'), 50),
                    valor=safe_numeric(row.get('VALOR')),
                    situacao=safe_str(row.get('SITUAÇÃO'), 100),
                    status_proposta=safe_str(row.get('STATUS PROPOSTA'), 100),
                    receita_espelhada=safe_numeric(row.get('RECEITA ESPELHADA')),
                    turma=safe_str(row.get('TURMA'), 100),
                    instrutor_1=safe_str(row.get('INSTRUTOR 1'), 255),
                    ch_c1=safe_str(row.get('CH C1'), 50),
                    instrutor_2=safe_str(row.get('INSTRUTOR 2'), 255),
                    ch_c2=safe_str(row.get('CH C2'), 50),
                    status_servico=safe_str(row.get('STATUS SERVIÇO'), 100),
                    codigo_material=safe_str(row.get('CÓDIGO MATERIAL'), 100),
                    observacoes=safe_str(row.get('Obs.')),
                    ano=safe_int(row.get('ANO')),
                    mes=safe_str(row.get('MÊS'), 20),
                    dados_iniciais=True
